...
const PurchaseReturns = () => {
  ...
  const loadReturns = async () => {
    try {
      setLoading(true);
      const res = await api.get('/api/purchase-returns/', { params: { page, per_page: 10 } });
  ...
  const loadPurchases = async () => {
    try {
      const res = await api.get('/api/purchases/', { params: { status: 'approved', per_page: 100 } });
      const raw2 = res.data?.data; setPurchases(Array.isArray(raw2) ? raw2 : (raw2?.items || []));
    } catch (err) {
    }
  };
  useEffect(() => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.purchase_id || formData.items.length === 0 || !formData.reason) {
      showError('Fill all required fields');
      return;
    }
    try {
      await api.post('/api/purchase-returns/', formData);
  ...
  const approveReturn = async (id) => {
    if (window.confirm('Approve this return?')) {
      try {
        await api.post(`/api/purchase-returns/${id}/approve`);
        showSuccess('Return approved. Inventory reversed.');
        loadReturns();
      } catch (err) {
  ...
  const resetForm = () => {
    setFormData({ purchase_id: '', reason: '', notes: '', items: [] });
    setCurrentItem({ material_id: '', quantity: '', unit_price: '' });
    setShowForm(false);
  };
  return (
    <div className="purchases-page theme-blue-white" style={{ background: 'linear-gradient(135deg, #f0f5ff 0%, #e8f0fe 100%)', minHeight: '100vh' }}>
      <div className="purchases-container">
        <div className="purchases-header">
          <h1 style={{ color: '#0052CC' }}>Purchase Returns</h1>
            ...
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit} className="purchase-form">
                <div className="form-group">
                  ...
                  <select required value={formData.purchase_id} onChange={(e) => setFormData({...formData, purchase_id: e.target.value})}>
